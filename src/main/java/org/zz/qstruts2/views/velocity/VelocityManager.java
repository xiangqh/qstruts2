...
import org.zz.qstruts2.views.util.ContextUtil;
...
public class VelocityManager {
	...
	public Context createContext(ActionContext ac, HttpServletRequest req, HttpServletResponse res) {
		...
		ServletContext ctx = (ServletContext) ac.get(ServletActionContext.SERVLET_CONTEXT);
		;
		try {
		...
		for (int i = 0; i < chainedContextNames.length; i++) {
			...
			try {
				...
				if (LOG.isWarnEnabled()) {
					LOG.warn("Warning.  " + e.getClass().getName()
							+ " caught while attempting to instantiate a chained VelocityContext, " + className
							+ " -- skipping");
				}
			}
		}
