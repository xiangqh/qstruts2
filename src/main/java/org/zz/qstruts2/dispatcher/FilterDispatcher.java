...
public class FilterDispatcher implements StrutsStatics, Filter {
	...
	/**
	 * Calls dispatcher.cleanup, which in turn releases local threads and
	 * destroys any DispatchListeners.
	 *
	 ...
	 * request parameter or any other unprotected method. Using a signed cookie
	 * is one safe way to turn it on per request.
	 *
	 * @param devMode
	 *            the override value
	 */
	...
	/**
	 * Create a default {@link Dispatcher} that subclasses can override with a
	...
	/**
	 * Create a default {@link Dispatcher} that subclasses can override with a
	 * custom Dispatcher, if needed. Called by createDispatcher(FilterConfig).
	 *
	 * @param ctx
	 *            ServletContext
	 * @param params
	 ...
	 * <p/>
	 * Servlet 2.3 specifies that the servlet context can be retrieved from the
	 * session. Unfortunately, some versions of WebLogic can only retrieve the
	 * servlet context from the filter config. Hence, this method enables
	 * subclasses to retrieve the servlet context from other sources.
	 *
	 ...
	 * <p/>
	 * The filter tries to match the request to an action mapping. If mapping is
	 * found, the action processes is delegated to the dispatcher's
	 * serviceAction method. If action processing fails, doFilter will try to
	 * create an error page via the dispatcher.
	 * <p/>
	 * Otherwise, if the request is for a static resource, the resource is
	 * copied directly to the response, with the appropriate caching headers
	 * set.
	 * <p/>
	 * If the request does not match an action mapping, or a static resource
	 * page, then it passes through.
	 *
