...
import org.zz.qstruts2.config.QStrutsXmlConfigurationProvider;
...
public class Dispatcher {
	 ...
	 * Introduced to allow integration with other frameworks like Spring
	 * Security
	 */
	...
	private void init_FileManager() throws ClassNotFoundException {
		if (initParams.containsKey(StrutsConstants.STRUTS_FILE_MANAGER)) {
			...
			configurationManager.addContainerProvider(new FileManagerProvider(fileManagerClass, fileManagerClass
					.getSimpleName()));
		} else {
		...
		for (String file : files) {
			if (file.endsWith(".xml")) {
				if ("xwork.xml".equals(file)) {
					...
					configurationManager.addContainerProvider(createStrutsXmlConfigurationProvider(file, false,
							servletContext));
				}
			} else {
	...
	private void init_QStrutsXmlConfigurations() {
		configurationManager.addContainerProvider(new QStrutsXmlConfigurationProvider("qstruts-plugin.xml", false,
				servletContext));
	}

	private void init_QStrutsAnnotationsConfigurations() {
		configurationManager.addContainerProvider(new RuntimeProvider());
	}
	...
	public void init() {
		...
		try {
			...
			init_QStrutsXmlConfigurations();
			init_QStrutsAnnotationsConfigurations();
	...
	public Map<String, Object> createContextMap(HttpServletRequest request, HttpServletResponse response,
		...
		Map<String, Object> session = new SessionMap<String, Object>(request);

		// application map wrapping the ServletContext
		Map<String, Object> application = new ApplicationMap(context);
	...
	public HashMap<String, Object> createContextMap(Map requestMap, Map parameterMap, Map sessionMap,
			Map applicationMap, HttpServletRequest request, HttpServletResponse response, ServletContext servletContext) {
	...
	public void sendError(HttpServletRequest request, HttpServletResponse response, ServletContext ctx, int code,
			Exception e) {
		Boolean devModeOverride = org.apache.struts2.dispatcher.FilterDispatcher.getDevModeOverride();
